...
function E2EEDevicesPanel({ userId }: { userId: string }) {
  ...
  return (
    <section className="mx-4 mt-8 rounded-[2rem] border border-border-subtle bg-surface/70 p-4 shadow-sm">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="text-base font-black text-foreground">Устройства</h3>
          <p className="mt-1 text-[11px] font-bold text-muted">Устройства с ключами сквозного шифрования</p>
        </div>
        <button onClick={() => void loadDevices()} className="rounded-full bg-primary/10 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-primary active:scale-95">
          Обновить
        </button>
      </div>
      {loading ? <p className="py-4 text-sm font-bold text-muted">Загрузка...</p> : null}
      {error ? <p className="mb-3 rounded-xl bg-red-500/10 px-3 py-2 text-xs font-bold text-red-500">{error}</p> : null}
      <div className="space-y-2">
        {devices.map((device) => {
          const revoked = Boolean(device.revokedAt);
          const title = device.name || device.platform || "Nox device";
          return (
            <div key={device.deviceId} className="rounded-2xl border border-border-subtle bg-background/50 p-3">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <p className="truncate text-sm font-black text-foreground">{title}</p>
                    {device.isCurrentDevice ? <span className="rounded-full bg-primary/10 px-2 py-0.5 text-[9px] font-black uppercase text-primary">Это устройство</span> : null}
                    {revoked ? <span className="rounded-full bg-red-500/10 px-2 py-0.5 text-[9px] font-black uppercase text-red-500">Отозвано</span> : null}
                  </div>
                  <p className="mt-1 text-[11px] font-semibold text-muted">{device.platform || "Web"} · {device.lastSeenAt ? `было ${new Date(device.lastSeenAt).toLocaleString("ru-RU")}` : "нет активности"}</p>
                  {device.fingerprintShort ? <p className="mt-1 break-all text-[10px] font-mono text-muted">{device.fingerprintShort}</p> : null}
                </div>
                {!device.isCurrentDevice && !revoked ? (
                  <button onClick={() => void revokeDevice(device)} className="shrink-0 rounded-full bg-red-500/10 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-red-500 active:scale-95">
                    Отозвать
                  </button>
                ) : null}
              </div>
